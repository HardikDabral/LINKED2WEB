...
const faqs = [
  { q: "Does the timer keep counting if I switch tabs?", a: "Yes. The countdown is based on the real clock, so even if the tab is backgrounded or your screen sleeps, the remaining time will be correct when you return." },
  { q: "Can I pause and resume the countdown?", a: "Yes — use the pause button to freeze the remaining time, and resume to continue from the same point." },
  { q: "Will I hear an alarm when time is up?", a: "Yes, an alert sound plays when the timer reaches zero. Make sure your browser tab isn’t muted and your system volume is on." },
  { q: "Does closing the browser reset the timer?", a: "Yes. The timer runs in-memory, so closing the tab or browser ends the countdown. For long-running timers, keep the tab open." },
]
