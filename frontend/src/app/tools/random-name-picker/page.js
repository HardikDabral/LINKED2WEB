...
const faqs = [
  { q: "How do I add names to the picker?", a: "Paste or type names (one per line) into the input, then press Pick. The tool accepts any list — students, team members, raffle entries, etc." },
  { q: "Is the selection truly random?", a: "Yes. We use the browser’s secure random number generator, so every name has an equal chance on every pick." },
  { q: "Can I exclude names that have already been picked?", a: "Yes. Enable the “remove after pick” option to run a draw without repeats — useful for giveaways and turn-taking." },
  { q: "Is the list saved between sessions?", a: "No. The list lives only in your browser tab. Close the tab or refresh and the entries are cleared, so paste them again if you want to re-run the draw." },
]
  ...
  <div className="max-w-2xl w-full mx-4 sm:mx-auto p-6 pt-16">
        ...
        <div className="mt-8 bg-white/50 backdrop-blur-xl rounded-xl p-6 shadow-md border border-[#93B1B5]/40">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Link href="/tools/decision-maker" className="text-[#0B2E33] hover:text-[#4F7C82] transition-colors text-sm">
              → Decision Maker
            </Link>
            <Link href="/tools/word-counter" className="text-[#0B2E33] hover:text-[#4F7C82] transition-colors text-sm">
              → Word Counter
            </Link>
            <Link href="/tools/todo-list" className="text-[#0B2E33] hover:text-[#4F7C82] transition-colors text-sm">
              → Todo List
            </Link>
